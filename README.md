...
- **Tags** (create, attach, detach), **notifications** (create/update providers, attach per
  monitor), **maintenance windows** (recurring, suppress alerts), and **status pages**
  (create/update, add monitors to public groups).
...
| POST | `/v1/notifications` | create/update a notification provider (e.g. Rocket.Chat) |
